...
func calculate_moveable_area(
	...
	var point_ids: PackedInt32Array = dijkstra_map.get_all_points_with_cost_between(
		0.0, max_cost
	)
...
func calculate_path_to_cell(cell_pos: Vector2i) -> PackedVector2Array:
	...
	for id in path_ids:
		new_path.push_back(
			map_to_local(_id_to_position[id])
		)
...
func highlight_cell_area(cell_positions: PackedVector2Array) -> void:
	...
	for cell_pos in cell_positions:
		set_cell(Layers.HIGHLIGHT, cell_pos, TILE_SET_SOURCE_ID, get_tileset_atlas_pos(Tiles.HIGHLIGHT))
