...
func generate_maze():
	...
	# Reset TileMap to a blank state
	tilemap.clear()
	...
	for i in tilemap_dimensions.x:
		for j in tilemap_dimensions.y:
			tilemap.set_cell(
				...
				Vector2i(i, j),
				TILE_SET_SOURCE_ID,
				TILE_ATLAS_COORDS[Tiles.WALL]
			)
	
	# Scale to fit new maze size
	var new_scale := Vector2(initial_dimensions) / Vector2(maze_dimensions)
	new_scale = Vector2(new_scale.x, new_scale.x) if new_scale.x < new_scale.y else Vector2(new_scale.y, new_scale.y)
	tilemap.scale = new_scale
	
	# Set up initial state of unvisited nodes
	...
	for i in maze_nodes.size():
		...
		for j in maze_nodes_inner.size():
			tilemap.set_cell(
				Layers.MAIN, 
				Vector2i((i * 2) + 1, (j * 2) + 1),
				TILE_SET_SOURCE_ID,
				TILE_ATLAS_COORDS[Tiles.GROUND]
			)
	
	var current_node := Vector2i(randi_range(0, maze_dimensions.x - 1), randi_range(0, maze_dimensions.y - 1))
	var node_queue := []
	...
	while true:
		...
		if unvisited_neighbor_offsets.size() == 0:
			while node_queue.size() > 0:
				var new_test_node = node_queue.pop_front()
		...
		var neighbor_offset = unvisited_neighbor_offsets[randi_range(0, unvisited_neighbor_offsets.size() - 1)]
...
func _get_unvisited_neighbor_offsets(origin_node_pos: Vector2i) -> Array[Vector2i]:
	...
	for offset: Vector2i in [Vector2i(-1, 0), Vector2i(1, 0), Vector2i(0, -1), Vector2i(0, 1)]:
		...
		if (new_pos.x < 0 || new_pos.x >= maze_dimensions.x 
			|| new_pos.y < 0 || new_pos.y >= maze_dimensions.y):
			continue
...
func _tilemap_cell_selected(cell: Vector2i) -> void:
	...
	if existing_cell_atlas_coords == Vector2i(-1, -1): # Empty cell
		if start_point == Vector2i(NAN, NAN):
			start_point = cell
			tilemap.set_cell(Layers.POINTS, cell, TILE_SET_SOURCE_ID, TILE_ATLAS_COORDS[Tiles.START])
		else:
...
func _solve_maze_for_dijkstra() -> void:
	...
	var _dijkstra_grid_points_to_ids := {}
	var _dijkstra_grid_ids_to_points := {}
	
	if use_grid_methods:
		_dijkstra_grid_points_to_ids = dijkstra_map.add_square_grid(
			Rect2(0, 0, tilemap_dimensions.x, tilemap_dimensions.y),
			-1,
			1.0,
			INF
		)
		# Update terrains for walls and ground tiles so pathfinding will be accurate
		for pos in _dijkstra_grid_points_to_ids.keys():
			var id: int = _dijkstra_grid_points_to_ids[pos]
			...
			_dijkstra_grid_ids_to_points[id] = pos
		
		dijkstra_start_point = _dijkstra_grid_points_to_ids[Vector2(start_point)]
		for end_point in end_points:
			dijkstra_end_points.push_back(_dijkstra_grid_points_to_ids[Vector2(end_point)])
		
		dijkstra_map.recalculate(dijkstra_start_point, {
		...
		for i in tilemap_dimensions.x - 1:
			for j in tilemap_dimensions.y - 1:
				...
				var tile_atlas_coords : Vector2i = tilemap.get_cell_atlas_coords(Layers.MAIN, tilemap_coords)
				var tile_type = TILE_ATLAS_COORDS.find_key(tile_atlas_coords)
				
				if tile_type == Tiles.GROUND:
					var dijkstra_id := tilemap_coords_to_dijkstra_id(tilemap_coords, tilemap_dimensions.x)
					...
					for neighbor_coords in top_and_left_neighbor_coords:
						var neighbor_id := tilemap_coords_to_dijkstra_id(neighbor_coords, tilemap_dimensions.x)
		...
		for end_point in end_points:
			dijkstra_end_points.push_back(tilemap_coords_to_dijkstra_id(end_point, tilemap_dimensions.x))
	...
	for solved_path in solved_paths:
		...
		for dijkstra_id in solved_path:
			...
			if use_grid_methods:
				cell_pos = Vector2i(_dijkstra_grid_ids_to_points[dijkstra_id])
			else:
				cell_pos = dijkstra_id_to_tilemap_coords(dijkstra_id, tilemap_dimensions.x)
			tilemap.set_cell(Layers.PATH, cell_pos, TILE_SET_SOURCE_ID, TILE_ATLAS_COORDS[Tiles.PATH])
	...
	# Use the AStar2D implementation
		...
		for i in tilemap_dimensions.x - 1:
			for j in tilemap_dimensions.y - 1:
				...
				if tile_type == Tiles.GROUND:
					var astar_id := tilemap_coords_to_dijkstra_id(tilemap_coords, tilemap_dimensions.x)
		...
		for end_point in end_points:
			astar_end_points.push_back(tilemap_coords_to_dijkstra_id(end_point, tilemap_dimensions.x))
...
func get_tile_type_from_cell_position(cell_pos: Vector2i) -> int:
	var tile_type: int = TILE_ATLAS_COORDS.find_key(tilemap.get_cell_atlas_coords(Layers.MAIN, cell_pos))
