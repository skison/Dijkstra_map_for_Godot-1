...
func test_duplicate_graph_from():
	...
	dijkstra_map.add_point(2, -1)
	dijkstra_map.add_point(3, -1)
	dijkstra_map.connect_points(1, 2, 1.0, true)
	var dijkstra_map_copy = DijkstraMap.new()
	dijkstra_map_copy.duplicate_graph_from(dijkstra_map)
	dijkstra_map.add_point(4, -1)
...
func test_add_point():
	var res: int
	var dijkstra_map = DijkstraMap.new()
	res = dijkstra_map.add_point(0, -1) # default terrain_type is -1
	assert_eq(res, OK)
...
# TODO: re-enable these once get_terrain_for_point is available in API
...
func test_connect_points():
	...
	assert_eq(dijkstra_map.connect_points(0, 1, 2.0, true), OK)
	# default weight is 1.0
	assert_eq(dijkstra_map.connect_points(1, 2, 1.0, true), OK)
	assert_eq(dijkstra_map.connect_points(1, 3, 1.0, false), OK)
	# produces the graph :
	# 0 <---> 1 <---> 2 ----> 3
	#    2.0     1.0     1.0
	assert_eq(dijkstra_map.connect_points(1, 4, 1.0, true), FAILED, "4 does not exists in the map")
	assert_eq(dijkstra_map.connect_points(1, 5, 1.0, true), FAILED, "5 does not exists in the map")
...
func test_remove_connection():
	...
	dijkstra_map.add_point(0, -1)
	dijkstra_map.add_point(1, -1)
	dijkstra_map.connect_points(0, 1, 1.0, true)
	assert_eq(dijkstra_map.remove_connection(0, 1, true), OK)
	assert_eq(dijkstra_map.remove_connection(0, 2, true), FAILED) # 2 does not exists in the map
	dijkstra_map.connect_points(0, 1, 1.0, true)
...
func test_recalculate():
	...
	dijkstra_map.connect_points(0, 1, 1.0, true)
	dijkstra_map.connect_points(1, 2, 10.0, true)
	var optional_params = {
