...
func recalculate() -> void:
	...
	var targets: Array = get_used_cells_by_id(Layers.MAIN, TileAtlases.MAIN, get_main_tileset_atlas_pos(MainTiles.ORIGIN))
	...
	for id in costs.keys():
		...
		set_cell(Layers.COSTS, id_to_pos[id], TileAtlases.GRADIENT, get_tileset_atlas_pos_for_cost_value(cost))
	...
	for id in dir_ids.keys():
		...
		set_cell(Layers.DIRECTIONS, id_to_pos[id], TileAtlases.MAIN, get_arrow_tile_atlas_pos_for_direction(dir))
...
func _on_visualization_selection_item_selected(index: int) -> void:
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if (event is InputEventMouseMotion or event is InputEventMouseButton) and dragging:
		...
		if cell.x >= 0 and cell.x < TILEMAP_RECT.size.x and cell.y >= 0 and cell.y < TILEMAP_RECT.size.y:
