...
func _ready():
	...
	gdscript_gridmap.process_mode = Node.PROCESS_MODE_INHERIT if !use_c_sharp else Node.PROCESS_MODE_DISABLED
	c_sharp_gridmap.visible = use_c_sharp
	c_sharp_gridmap.process_mode = Node.PROCESS_MODE_INHERIT if use_c_sharp else Node.PROCESS_MODE_DISABLED
...
func _on_terrain_selection_item_selected(index):
	if use_c_sharp:
		...
		gdscript_gridmap._on_terrain_selection_item_selected(index)
...
func _on_visualization_selection_item_selected(index):
	if use_c_sharp:
		...
		gdscript_gridmap._on_visualization_selection_item_selected(index)
